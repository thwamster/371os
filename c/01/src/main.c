...
int main(const int argc, char ** argv) {
	...
	for (int i = 1; i < argc; i++) {
		...
		if (parse_argument(arg) == 1) { return 0; }
	}
	input();
	output();
...
void output() {
	for (int i = 1; i <= file_count; i++) {
		...
		for (int j = 0; j < 5; j++) {
			if (file_data[0][j] >= 0) { printf(" %*d", file_data[file_count + 1][j], file_data[i][j]); }
		}
	...
	if (file_count > 1) {
		for (int j = 0; j < 5; j++) {
			if (file_data[0][j] >= 0) { printf(" %*d", file_data[file_count + 1][j], file_data[0][j]); }
		}
		printf(" total\n");
	}
}
...
int parse_argument(const char * s) {
	if (strcmp(s, "-") != 0) {
		if (strncmp(s, "--", 2) == 0) { return parse_option_long(s + 2); }
		if (strncmp(s, "-", 1) == 0) { return parse_option_short(s + 1); }
	}
...
int parse_file(const char * s) {
	if (enable_files0 == 1) { return throw_error(s, EXTRA_OPERAND); }
...
int parse_files0(FILE * f) {
	...
	while ((c = (char) getc(f)) != EOF) {
		if (c == '\0') {
			...
			if (now == NULL) { return throw_error(file_paths[file_count], CANNOT_OPEN); }
			fclose(now);
		}
...
int parse_option_long(const char * s) {
	...
	if (strncmp(s, "files0-from=", 12) == 0) {
		if (enable_files0 == 0) { return throw_error(s, EXTRA_OPERAND); }

		enable_files0 = 1;

		FILE * file = (strcmp(s, "-") == 0) ? file = stdin : fopen(s + 12, "r");
		if (file == NULL) { return throw_error(s, NO_SUCH); }
		return parse_files0(file);
	}
	if (strcmp(s, "bytes") == 0) { enable_option(BYTES); }
	else if (strcmp(s, "chars") == 0) { enable_option(CHARS); }
	else if (strcmp(s, "lines") == 0) { enable_option(NEWLINES); }
	else if (strcmp(s, "max-line-length") == 0) { enable_option(MAX_LINE_LENGTH); }
	else if (strcmp(s, "words") == 0) { enable_option(WORDS); }
	else { return throw_error(s, UNRECOGNIZED_OPTION); }
...
int parse_option_short(const char * s) {
	while (strlen(s) > 0) {
		...
		else if (strncmp(s, "L", 1) == 0) { enable_option(MAX_LINE_LENGTH); }
		else if (strncmp(s, "w", 1) == 0) { enable_option(WORDS); }
		else { return throw_error(s, INVALID_OPTION); }
...
void analyze_file(FILE * f, const int n) {
	...
	while ((c = (char) getc(f)) != EOF) {
		...
		if (c == '\t') { line_length += 8 - (line_length % 8); }
		else if (c == '\n') {
			line_length = line_length > 0 ? line_length : 0;
			if (file_data[n][MAX_LINE_LENGTH] < line_length) { file_data[n][MAX_LINE_LENGTH] = line_length; }
			file_data[n][NEWLINES] += 1;
			line_length = 0;
		}
		else { line_length += 1; }

		if (!isspace(c)) { word_length += 1; }
		else {
			if (word_length > 0) {
				file_data[n][WORDS] += 1;
				word_length = 0;
	...
	if (word_length > 0) { file_data[n][WORDS] += 1; }

	for (int i = 0; i < 4; i++) {
		if (file_data[0][i] >= 0) {
			file_data[0][i] += file_data[n][i];
			if (file_data[0][i] > 0) { file_data[file_count + 1][i] = (int) floor(log10(abs(file_data[0][i]))) + 1; }
		}
	}
	...
	if (file_data[0][MAX_LINE_LENGTH] > 0) { file_data[file_count + 1][MAX_LINE_LENGTH] = (int) floor(log10(abs(file_data[0][MAX_LINE_LENGTH]))) + 1; }
}
